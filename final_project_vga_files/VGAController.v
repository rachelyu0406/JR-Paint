...
module VGAController(     
	...
	localparam 
		...
		PIXEL_ADDRESS_WIDTH = $clog2(PIXEL_COUNT) + 1,           
		BITS_PER_COLOR = 12, 	  								 
	...
	wire[BITS_PER_COLOR-1:0] colorSelect; 			  // Output color 
	wire[BITS_PER_COLOR-1:0] colorOut;
	assign colorSelect = in_bounds ? sprite_color : colorData; 
	assign colorOut = active ? colorSelect : 12'd0; // When not active, output black
	...
	wire in_bounds; 
	assign in_bounds = ((x>=sprite_x_ref) & (x<=sprite_x_ref + 50)) & ((y>=sprite_y_ref) & (y<=sprite_y_ref + 50));
	...
	/* ------------------------ Character Fetching ------------------------ */
	Ps2Interface USB_to_Ps2(.ps2_clk(ps2_clk), .ps2_data(ps2_data), .rst(reset), .clk(clk),
	                        .tx_data(8'd0), .write_data(1'b0), 
	                        .rx_data(new_scancode), .read_data(data_available)); 
